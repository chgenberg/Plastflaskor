...
import { SHIPPED_MEMORY } from "@/lib/orchestrator/memory-shipped";

const CURSOR_DIR = ".cursor/orchestrator";
const SHIPPED_MEMORY_REL = "src/lib/orchestrator/MEMORY.md";

export function cursorOrchestratorDir(root = process.cwd()): string {
  return path.join(root, CURSOR_DIR);
}

export function memoryFile(root = process.cwd()): string {
  return path.join(root, CURSOR_DIR, "MEMORY.md");
}

export function shippedMemoryFile(root = process.cwd()): string {
  return path.join(root, SHIPPED_MEMORY_REL);
}

export function dailyNoteFile(isoDate: string, root = process.cwd()): string {
...
export async function readMemory(root = process.cwd()): Promise<{
  body: string;
...
export async function readDaily(isoDate: string, root = process.cwd()): Promise<string> {
  return readUtf8(dailyNoteFile(isoDate, root));
}

export async function appendDaily(line: string, now = new Date(), root = process.cwd()): Promise<void> {
...
export async function promoteToMemory(paragraph: string, root = process.cwd()): Promise<void> {
